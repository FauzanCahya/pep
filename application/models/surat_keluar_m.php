...
class Surat_keluar_m extends CI_Model
{
	...
	function simpan_data_barang_detail($id_permintaan_baru,$id_produk, $nama_produk,$keterangan,$kuantitas,$satuan)
	{
		...
		$sql = "
			INSERT INTO tb_surat_keluar_detail (
				id_induk,
				id_produk,
				nama_produk,
				keterangan,
	...
	{
		$sql = "SELECT * FROM tb_surat_keluar WHERE id_permintaan = '$id' ";
	...
	{
		$sql = "SELECT * FROM tb_surat_keluar_detail WHERE id_induk = '$id' ";
		$query = $this->db->query($sql);
		return $query->result();
	}
	...
	function ubah_data_permintaan_detail($id,$nama_produk,$keterangan,$kuantitas,$satuan,$harga,$jumlah)
	{
		...
		$sql = "
			UPDATE tb_surat_keluar_detail SET 
    ...
    function get_data_trx($id){
    	$sql = "
        SELECT pb.* , md.nama_divisi FROM tb_surat_keluar pb , master_divisi md WHERE pb.divisi = md.id_divisi AND pb.id_permintaan = '$id'
        ";
    ...
    function get_data_trx_detail($id){
    	$sql = "
        SELECT * FROM tb_surat_keluar_detail WHERE id_induk = '$id'
        ";
