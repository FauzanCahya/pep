...
class Pengembalian_barang_m extends CI_Model
{
	...
	function simpan_data_barang_detail($id_pengembalian_baru,$id_produk, $nama_produk,$keterangan,$kuantitas,$satuan,$harga,$jumlah)
	{
	...
	{
		$sql = "DELETE FROM  tb_pengembalian_barang WHERE id_pengembalian = '$id' " ;
		$this->db->query($sql);
    ...
    function get_transaction_info($id_barang){
        $sql = "
        SELECT mb.kode_barang , mb.nama_barang , pbd.sisa_jumlah , pbd.satuan , pb.no_spb FROM master_barang mb , tb_peminjaman_barang pb , tb_peminjaman_barang_detail pbd WHERE mb.id_barang = pbd.id_produk AND pb.id_peminjaman = pbd.id_induk AND pb.divisi = '$id_barang'
        ";
