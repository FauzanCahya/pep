...
$(document).ready(function(){
	...
	$("#tambah_surat_keluar").click(function(){
		$("#tambah_surat_keluar").fadeOut('slow');
		$("#table_surat_keluar").fadeOut('slow');
		$("#form_surat_keluar").fadeIn('slow');
		$("#tabel_total").fadeIn('slow');
	});

	$("#batal").click(function(){
		$("#tambah_surat_keluar").fadeIn('slow');
		$("#table_surat_keluar").fadeIn('slow');
		$("#form_surat_keluar").fadeOut('slow');
...
function hapus_permintaan(id)
{
	...
	$('#popup_hapus').show();
		...
		$.ajax({
		url : '<?php echo base_url(); ?>surat_keluar_c/data_permintaan_id',
		data : {id:id},
		type : "POST",
		dataType : "json",
		async : false,
...
function ubah_detail(id){
	$.ajax({
		url : '<?php echo base_url(); ?>surat_keluar_c/data_permintaan_detail_id',
		data : {id:id},
		type : "POST",
		dataType : "json",
		async : false,
...
function ajax_produk(id_form){
    ...
    $.ajax({
        url : '<?php echo base_url(); ?>surat_keluar_c/get_produk_popup',
        type : "POST",
        dataType : "json",
        data : {
            keyword : keyword,
...
function get_produk_detail(id, no_form){
	var id_produk = id;
    $.ajax({
		url : '<?php echo base_url(); ?>surat_keluar_c/get_produk_detail',
		data : {id_barang:id},
		type : "GET",
		dataType : "json",
		success : function(result){
...
function simpan_add_produk(){
	...
	if(nama_produk == ""){
		...
		$.ajax({
			url : '<?php echo base_url(); ?>surat_keluar_c/simpan',
			data : {
...
<div class="row" id="form_surat_keluar" style="display:none; ">
				...
				<div class="row" style="padding-top: 35px; padding-bottom: 15px;">
					<div class="col-md-12">
						<div class="col-md-offset-2 col-md-10">
							...
							<button type="button" id="batal" class="btn red" onclick="window.location = '<?php echo base_url(); ?>surat_keluar_c'">Batal Dan Kembali</button>
						</div>
					</div>
				</div>
			</div>
...
<button id="tambah_surat_keluar" class="btn green">
Tambah Data Surat Keluar <i class="fa fa-plus"></i>
</button>
</br>
</br>

<div class="row" id="table_surat_keluar" style="display:block;">
	<div class="col-md-12">
		<!-- BEGIN EXAMPLE TABLE PORTLET-->
		<div class="portlet box green">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-edit"></i>Table Surat Barang Keluar
				</div>
				<div class="tools">
			...
			<div class="portlet-body">
				...
				<tr>
					...
					<td style="text-align:center; vertical-align: middle;">
						...
						<!-- <a target="_blank" class="btn default btn-xs green" id="hapus" href="<?=base_url();?>surat_keluar_c/cetak/<?=$value->id_permintaan;?>" ><i class="fa fa-print"></i> Cetak </a> -->
					</td>
				</tr>
					<?php 
						}
