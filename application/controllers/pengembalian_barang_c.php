	...
	function simpan()
	{
		...
		if ($id_pengembalian == '') {
			...
			foreach ($nama_produk as $key => $val) {
					 $this->pengembalian->simpan_data_barang_detail($id_pengembalian_baru,$id_produk[$key],$val,$keterangan[$key],$kuantitas[$key],$satuan[$key],$harga[$key],$jumlah[$key]);
			}
			...
			$this->pengembalian->ubah_data_pengembalian($id,$no_spb,$tanggal,$uraian);
			...
			foreach ($nama_produk as $key => $val) {
				$this->pengembalian->ubah_data_pengembalian_detail($id,$val,$keterangan[$key],$kuantitas[$key],$satuan[$key],$harga[$key],$jumlah[$key]);
			}
			$this->session->set_flashdata('sukses','1');
			redirect('pengembalian_barang_c');
		}
