...
class Bon_gudang_c extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('bon_gudang_m','bon_gudang');
	...
	public function index()
	{
		$data = array(
				'title' 	      => 'Bon Gudang',
				'page'  	      => 'bon_gudang_v',
				'sub_menu' 	      => 'Bon Gudang Sementara',
				'sub_menu1'	      => 'Bon Gudang Sementara',
				'menu' 	   	      => 'bon',
				'menu2'		      => 'bon',
				'lihat_data'      => $this->bon_gudang->lihat_data_bon_gudang(),
				'lihat_barang'    => $this->bon_gudang->lihat_data_barang(),
				'dt_dept'   	  => $this->bon_gudang->lihat_data_divisi(),
				'url_simpan' 	  => base_url().'bon_gudang_c/simpan',
				'url_hapus'  	  => base_url().'bon_gudang_c/hapus',
			);
	...
	function simpan()
	{
		...
		$id_bon_gudang     = $this->input->post('id_bon_gudang');
		if ($id_bon_gudang == '') {
			...
			$get_nomor	   = $this->master_model_m->get_nomor_dokumen('BON_GUDANG_SEMENTARA');

			$no_bukti_real = $get_nomor."/BGS/".$dept_row->nama_divisi."/".$var."/".$tahun_kas;
			$tanggal 	  = $this->input->post('tanggal');
			$uraian 	  = $this->input->post('uraian');

			$this->master_model_m->update_nomor('BON_GUDANG_SEMENTARA');
			$this->bon_gudang->simpan_data_barang($no_bukti_real,$tanggal,$uraian,$nama,$departemen);
			

			$id_bon_gudang_baru = $this->db->insert_id();
			...
			foreach ($nama_produk as $key => $val) {
				 $this->bon_gudang->simpan_data_barang_detail($id_bon_gudang_baru,$produk[$key],$val,$keterangan[$key],$kuantitas[$key],$satuan[$key],$reff_no[$key]);
			}

			// foreach ($id_peminjaman_detail as $keyi => $vali) {
			// 	$this->bon_gudang->update_selisih_detail($vali,$kuantitas[$keyi]);
			// }
			...
			$sql_buk = "SELECT NEXT_NOMOR FROM ak_nomor WHERE TIPE = 'BON_GUDANG_SEMENTARA'";
			...
			$this->bon_gudang->save_next_nomor('bon_gudang_DUA');
			$this->bon_gudang->simpan_data_barang($no_bukti_real,$tanggal,$uraian,$nama,$departemen);
	...
	function hapus()
	{
		...
		$this->bon_gudang->hapus_bon_gudang($id);
	...
	function data_bon_gudang_id()
	{
		...
		$data = $this->bon_gudang->data_bon_gudang_id($id);
		echo json_encode($data);
	}

	function data_bon_gudang_detail_id()
	{
		$id = $this->input->post('id');
		$data = $this->bon_gudang->data_bon_gudang_detail_id($id);
		echo json_encode($data);	
	}
	...
	function get_produk_detail(){
		...
		$dt = $this->bon_gudang->get_produk_detail($id_barang);
	...
	function get_transaction_info(){
		$id = $this->input->post('id');
		$dt = $this->bon_gudang->get_transaction_info($id);
